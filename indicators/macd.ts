import type { Candle, MacdResult, TimeSeriesPoint } from "./types.js";
...
export function computeMacd(
  ...
  const macdRaw: Array<number | undefined> = candles.map((_, i) => {
    const f = emaFast[i]?.value;
    const s = emaSlow[i]?.value;
    if (f == null || s == null || !Number.isFinite(f) || !Number.isFinite(s)) return undefined;
    return f - s;
  });
  ...
  const signalRaw: Array<number | undefined> = new Array(candles.length);
  let sum = 0;
  ...
  const histogram: TimeSeriesPoint[] = candles.map((c, i) => {
    const m = macdRaw[i];
    const s = signalRaw[i];
