...
export const PORTFOLIO_PROXY_METHODOLOGY =
  "Portfolio Value = reported 13F filing total in USD for the primary filing in each quarter (largest reported total). Filings whose holdings mostly still look like SEC thousands (implied price value/shares < $1) are scaled ×1000; residual ~1000× cliffs between adjacent quarters with stable holdings counts are normalized the same way. QoQ / 1Y / 3Y compare those normalized USD totals only; missing quarters are N/A and never interpolated. Percent growth is suppressed when the prior book is below $10M, is less than 33% of the current book, or is ≥100% (typical of capital inflows or mandate changes — not investment return). This is a reported-value proxy, not an investment return.";
