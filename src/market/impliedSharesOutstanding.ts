...
let yahooFinance: YahooFinance | null = null;

function getYahooFinance(): YahooFinance {
  if (!yahooFinance) {
...
/** Yahoo `defaultKeyStatistics.impliedSharesOutstanding` for a ticker. */
export async function fetchImpliedSharesOutstanding(symbol: string): Promise<number | null> {
  ...
  const value = yahooNum(data.defaultKeyStatistics?.impliedSharesOutstanding);
