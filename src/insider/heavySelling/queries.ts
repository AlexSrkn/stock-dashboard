...
/**
 * Open-market sells only (Form 4 code S, non-derivative).
 * Excludes options/RSUs/grants/gifts/tax withholding (F)/conversions.
 */
export const SELECT_OPEN_MARKET_SELLS_SQL = `
SELECT
  it.id,
...
WHERE it.ticker IS NOT NULL
  ...
  AND UPPER(BTRIM(it.transaction_code)) = 'S'
  AND (
    it.acquisition_disposition IS NULL
    OR UPPER(BTRIM(it.acquisition_disposition)) = 'D'
  )
...
export function isPlausibleOpenMarketSell(row: RawOpenMarketSell): boolean {
...
export async function loadOpenMarketSells(
  pool: pg.Pool = getPool(),
  opts: { dateFrom?: string | null; dateTo?: string | null } = {}
): Promise<RawOpenMarketSell[]> {
  ...
  const sql =
    conditions.length > 0
      ? `${SELECT_OPEN_MARKET_SELLS_SQL} AND ${conditions.join(" AND ")}`
      : SELECT_OPEN_MARKET_SELLS_SQL;

  const res = await pool.query<RawOpenMarketSell>(sql, params);
  return res.rows.filter(isPlausibleOpenMarketSell);
}
