...
/** Sanity bounds for open-market Form 4 rows (bad parses often put face amounts in price). */
...
/**
 * Strict open-market purchase filter (SEC Form 4):
 * - transaction_code = P (open-market / private purchase)
 * - non-derivative only (excludes options, convertibles, etc.)
 * - acquired (A) when disposition code is present
 *
 * Codes A/M/F/G/C and derivatives are excluded by this filter.
 * Rule 10b5-1 plans are not reliably tagged in ingested rows; they remain
 * indistinguishable from discretionary P trades without footnote parsing.
 */
...
/** Optional market-cap proxy via fundamentals shares outstanding + implied price from value/shares. */
