...
import { EvToolsError, getEvInputs, runEvCalculate } from "../tools/ev/index.js";

loadEnvFile();

const GET_RE = /^\/api\/tools\/ev\/([^/]+)\/?$/;
const CALC_RE = /^\/api\/tools\/ev\/calculate\/?$/;
...
async function readJsonBody(req: http.IncomingMessage): Promise<unknown> {
  ...
  try {
    ...
    throw new EvToolsError(400, "invalid_json", "Request body must be valid JSON.");
  }
}

export async function tryHandleToolsEv(
  url: URL,
  ...
  if (CALC_RE.test(url.pathname) && req.method === "POST") {
    try {
      ...
      json(res, 200, runEvCalculate(body));
    } catch (err) {
      if (err instanceof EvToolsError) {
        json(res, err.status, { error: err.code, message: err.message });
        return true;
      }
      const message = err instanceof Error ? err.message : String(err);
      json(res, 500, { error: "ev_calculate_error", message });
    }
    return true;
  }
  ...
  try {
    const payload = await getEvInputs(ticker);
    ...
    json(res, 500, { error: "ev_inputs_error", message });
  }
  return true;
}
