...
import {
  EvEbitdaToolsError,
  getEvEbitdaInputs,
  runEvEbitdaCalculate,
} from "../tools/evebitda/index.js";

loadEnvFile();

const GET_RE = /^\/api\/tools\/ev-ebitda\/([^/]+)\/?$/;
const CALC_RE = /^\/api\/tools\/ev-ebitda\/calculate\/?$/;
...
async function readJsonBody(req: http.IncomingMessage): Promise<unknown> {
  ...
  try {
    ...
    throw new EvEbitdaToolsError(400, "invalid_json", "Request body must be valid JSON.");
  }
}

export async function tryHandleToolsEvEbitda(
  url: URL,
  ...
  if (CALC_RE.test(url.pathname) && req.method === "POST") {
    try {
      ...
      json(res, 200, runEvEbitdaCalculate(body));
    } catch (err) {
      if (err instanceof EvEbitdaToolsError) {
        json(res, err.status, { error: err.code, message: err.message });
        return true;
      }
      const message = err instanceof Error ? err.message : String(err);
      json(res, 500, { error: "ev_ebitda_calculate_error", message });
    }
    return true;
  }
  ...
  try {
    const payload = await getEvEbitdaInputs(ticker);
    ...
    json(res, 500, { error: "ev_ebitda_inputs_error", message });
  }
  return true;
}
