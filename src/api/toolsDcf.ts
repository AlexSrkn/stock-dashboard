...
import {
  ...
  getDcfFilingInputs,
  runDcfCalculate,
} from "../tools/dcf/service.js";

loadEnvFile();

const GET_RE = /^\/api\/tools\/dcf\/([^/]+)\/?$/;
const CALC_RE = /^\/api\/tools\/dcf\/calculate\/?$/;
...
async function readJsonBody(req: http.IncomingMessage): Promise<unknown> {
  ...
  try {
    ...
    throw new DcfToolsError(400, "invalid_json", "Request body must be valid JSON.");
  }
}

export async function tryHandleToolsDcf(
  url: URL,
  ...
  if (CALC_RE.test(url.pathname) && req.method === "POST") {
    try {
      ...
      const result = runDcfCalculate(body);
      json(res, 200, result);
    } catch (err) {
      if (err instanceof DcfToolsError) {
        json(res, err.status, { error: err.code, message: err.message });
        return true;
      }
      const message = err instanceof Error ? err.message : String(err);
      json(res, 500, { error: "dcf_calculate_error", message });
    }
    return true;
  }
  ...
  try {
    const payload = await getDcfFilingInputs(ticker);
    ...
    json(res, 500, { error: "dcf_inputs_error", message });
  }
  return true;
}
