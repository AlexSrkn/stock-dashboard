...
import { PeToolsError, getPeInputs, runPeCalculate } from "../tools/pe/index.js";

loadEnvFile();

const GET_RE = /^\/api\/tools\/pe\/([^/]+)\/?$/;
const CALC_RE = /^\/api\/tools\/pe\/calculate\/?$/;
...
async function readJsonBody(req: http.IncomingMessage): Promise<unknown> {
  ...
  try {
    ...
    throw new PeToolsError(400, "invalid_json", "Request body must be valid JSON.");
  }
}

export async function tryHandleToolsPe(
  url: URL,
  ...
  if (CALC_RE.test(url.pathname) && req.method === "POST") {
    try {
      ...
      json(res, 200, runPeCalculate(body));
    } catch (err) {
      if (err instanceof PeToolsError) {
        json(res, err.status, { error: err.code, message: err.message });
        return true;
      }
      const message = err instanceof Error ? err.message : String(err);
      json(res, 500, { error: "pe_calculate_error", message });
    }
    return true;
  }
  ...
  try {
    const payload = await getPeInputs(ticker);
    ...
    json(res, 500, { error: "pe_inputs_error", message });
  }
  return true;
}
