...
import { WaccToolsError, getWaccInputs, runWaccCalculate } from "../tools/wacc/index.js";

loadEnvFile();

const GET_RE = /^\/api\/tools\/wacc\/([^/]+)\/?$/;
const CALC_RE = /^\/api\/tools\/wacc\/calculate\/?$/;
...
async function readJsonBody(req: http.IncomingMessage): Promise<unknown> {
  ...
  try {
    ...
    throw new WaccToolsError(400, "invalid_json", "Request body must be valid JSON.");
  }
}

export async function tryHandleToolsWacc(
  url: URL,
  ...
  if (CALC_RE.test(url.pathname) && req.method === "POST") {
    try {
      ...
      json(res, 200, runWaccCalculate(body));
    } catch (err) {
      if (err instanceof WaccToolsError) {
        json(res, err.status, { error: err.code, message: err.message });
        return true;
      }
      const message = err instanceof Error ? err.message : String(err);
      json(res, 500, { error: "wacc_calculate_error", message });
    }
    return true;
  }
  ...
  try {
    const payload = await getWaccInputs(ticker);
    ...
    json(res, 500, { error: "wacc_inputs_error", message });
  }
  return true;
}
