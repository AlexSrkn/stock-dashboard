...
import { EpvToolsError, getEpvInputs, runEpvCalculate } from "../tools/epv/index.js";

loadEnvFile();

const GET_RE = /^\/api\/tools\/epv\/([^/]+)\/?$/;
const CALC_RE = /^\/api\/tools\/epv\/calculate\/?$/;
...
async function readJsonBody(req: http.IncomingMessage): Promise<unknown> {
  ...
  try {
    ...
    throw new EpvToolsError(400, "invalid_json", "Request body must be valid JSON.");
  }
}

export async function tryHandleToolsEpv(
  url: URL,
  ...
  if (CALC_RE.test(url.pathname) && req.method === "POST") {
    try {
      ...
      json(res, 200, runEpvCalculate(body));
    } catch (err) {
      if (err instanceof EpvToolsError) {
        json(res, err.status, { error: err.code, message: err.message });
        return true;
      }
      const message = err instanceof Error ? err.message : String(err);
      json(res, 500, { error: "epv_calculate_error", message });
    }
    return true;
  }
  ...
  try {
    const payload = await getEpvInputs(ticker);
    ...
    json(res, 500, { error: "epv_inputs_error", message });
  }
  return true;
}
